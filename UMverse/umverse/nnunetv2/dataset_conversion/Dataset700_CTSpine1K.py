...
from nnunetv2.configuration import default_num_threads
from nnunetv2.dataset_conversion.Task056_Verse_normalize_orientation import normalize_slice_orientation, \
    read_image, \
...
def evaluate_verse_folder(folder_pred, folder_gt, out_json="/user94/shijian/U-Mamba/data/nnUNet_raw/verse.json"):
...
if __name__ == "__main__":
    base = "/user94/shijian/U-Mamba/data/nnUNet_raw/CTSpine1K"
    base_corrOrient = "/user94/shijian/U-Mamba/data/nnUNet_raw/CTSpine1K/dataset_corrOrient/"

    if not os.path.exists(base_corrOrient):
        os.makedirs(base_corrOrient)
    # correct orientation
    train_files_base = subfiles(join(base, "train"), join=False, suffix=".nii.gz")
    train_segs = [i[:-len(".nii.gz")] + ".nii.gz" for i in train_files_base]
    train_data = [i[:-len(".nii.gz")] + ".nii.gz" for i in train_files_base]
    test_files_base = [i[:-len(".nii.gz")] for i in subfiles(join(base, "test/images"), join=False, suffix=".nii.gz")]
    ...
    test_files_base = [i[:-len(".nii.gz")] for i in
                       subfiles(join(base_corrOrient, "test/images"), join=True, suffix=".nii.gz")]
    test_data = [i + ".nii.gz" for i in test_files_base]
    ...
    train_patient_names = [i[:-len("_seg.nii.gz")] for i in
                           subfiles(join(base_corrOrient, "train"), join=False, suffix="_seg.nii.gz")]
    ...
    folder_gt = "/user94/shijian/U-Mamba/data/nnUNet_raw/Dataset700_CTSpine1K/labelsTr"

    folder_pred = "/home/username/nnUnet/results/3d_fullres/Task056_VerSe/nnUNetTrainerV2__nnUNetPlansv2.1/cv_niftis_raw"
    out_json = "/home/username/nnUnet/results/3d_fullres/Task056_VerSe_3d_fullres_summary.json"
    evaluate_verse_folder(folder_pred, folder_gt, out_json)

    folder_pred = "/home/username/nnUnet/results/3d_lowres/Task056_VerSe/nnUNetTrainerV2__nnUNetPlansv2.1/cv_niftis_raw"
    out_json = "/home/username/nnUnet/results/3d_lowres/Task056_VerSe_3d_lowres_summary.json"
    evaluate_verse_folder(folder_pred, folder_gt, out_json)

    folder_pred = "/home/username/nnUnet/results/3d_cascade_fullres/Task056_VerSe/nnUNetTrainerV2CascadeFullRes__nnUNetPlansv2.1/cv_niftis_raw"
    out_json = "/home/username/nnUnet/results/3d_cascade_fullres/Task056_VerSe_3d_cascade_fullres_summary.json"
    evaluate_verse_folder(folder_pred, folder_gt, out_json)