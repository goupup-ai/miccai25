...
class RobustCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        out = super().forward(input, target.long())
        #ce_weight = [0.25] + [0.75] * 7 + [1.25] * 12 + [0.75] * 6  # 背景类的权重为0.25，其余为0.75
        #normalized_ce_weight = [w / sum(ce_weight) for w in ce_weight]
        #ce_weight = torch.tensor(normalized_ce_weight, device='cuda:0')
        #weighted_ce = out * ce_weight
        return out
