...
from nnunetv2.training.lr_scheduler.polylr import PolyLRScheduler, StepHalvingLRScheduler, CustomLRScheduler, WarmUpPolyLRScheduler
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.log_dir = join(self.output_folder, f'runs/experiment')  # TensorBoard日志保存路径
...
##########
    ...
    def configure_optimizers(self):
        ...
        # lr_scheduler = StepHalvingLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        # lr_scheduler = StepHalvingLRScheduler(optimizer, self.initial_lr)
        lr_scheduler = CustomLRScheduler(optimizer, self.initial_lr)
        # lr_scheduler = WarmUpPolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
    ...
    def get_dataloaders(self):
        ...
        if allowed_num_processes == 0:
            ...
            mt_gen_val = LimitedLenWrapper(self.num_val_iterations_per_epoch, data_loader=dl_val,
                                           ...
                                           wait_time=0.02)
            # mt_gen_train = LimitedLenWrapper(self.num_iterations_per_epoch, data_loader=dl_tr, transform=tr_transforms,
            #                                  num_processes=allowed_num_processes, num_cached=2, seeds=None,
            #                                  pin_memory=False, wait_time=0.02)
            # mt_gen_val = LimitedLenWrapper(self.num_val_iterations_per_epoch, data_loader=dl_val,
            #                                transform=val_transforms, num_processes=max(1, allowed_num_processes // 2),
            #                                num_cached=1, seeds=None, pin_memory=False,
            #                                wait_time=0.02)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # global_dc_per_class = [
        #    0.0 if tp == 0 and (fp > 0 or fn > 0) else
        #    1.0 if tp == 0 and fp == 0 and fn == 0 else
        #    2 * tp / (2 * tp + fp + fn)
    ...
    def on_epoch_end(self):
        ...
        Pseudo_dice = [np.round(i, decimals=4) for i in
                                               self.logger.my_fantastic_logging['dice_per_class_or_region'][-1]]
        first_seven = Pseudo_dice[:7]    
        middle_twelve = Pseudo_dice[7:19]     
        last_six = Pseudo_dice[-6:]   
        self.print_to_log_file('------Pseudo dice', Pseudo_dice)
    
        # 计算每个部分的平均值
        mean_first_seven = np.mean(first_seven)
        mean_middle_twelve = np.mean(middle_twelve)
        mean_last_six = np.mean(last_six)  

        self.print_to_log_file('mean_first_seven', np.round(mean_first_seven, decimals=4))
        self.print_to_log_file('mean_middle_twelve', np.round(mean_middle_twelve, decimals=4))
        self.print_to_log_file('mean_last_six', np.round(mean_last_six, decimals=4))
                                 
        self.print_to_log_file('mean_fg_dice',
                               np.round(self.logger.my_fantastic_logging['mean_fg_dice'][-1], decimals=4))
        self.print_to_log_file(
            f"Epoch time: {np.round(self.logger.my_fantastic_logging['epoch_end_timestamps'][-1] - self.logger.my_fantastic_logging['epoch_start_timestamps'][-1], decimals=2)} s")
        self.writer.add_scalar('Loss/train', np.round(self.logger.my_fantastic_logging['train_losses'][-1], decimals=4), self.current_epoch)
        self.writer.add_scalar('Accuracy/train', np.round(self.logger.my_fantastic_logging['mean_fg_dice'][-1], decimals=4), self.current_epoch)
        self.writer.add_scalar('LR/epoch', np.round(self.optimizer.param_groups[0]['lr'], decimals=5), self.current_epoch)
        self.writer.close()
...
#########################  
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'lr_scheduler_state': self.lr_scheduler.state_dict() if self.lr_scheduler is not None else None,  # 保存学习率调度器的状态
                }
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        # match. Use heuristic to make it match
            ...
            if 'lr_scheduler_state' in checkpoint and checkpoint['lr_scheduler_state'] is not None:
                self.lr_scheduler.load_state_dict(checkpoint['lr_scheduler_state'])
            print(f"Checkpoint found at {filename_or_checkpoint}. Resuming training from epoch {self.current_epoch}.")
            return True
        else:
            print(f"No checkpoint found at {filename_or_checkpoint}. Starting training from scratch.")
            return False
...
#########################################        
    ...
    def run_training(self):
        ...
        if os.path.isfile(checkpoint_latest):
            ...
            print(f"No checkpoint found at {checkpoint_latest}. Starting training from scratch.")
            resume_training = False
        if resume_training:
        # Continue training from checkpoint, adjust learning rate, etc.
            self.start_epoch = self.current_epoch
        else:
            # 如果没有 checkpoint，则初始化训练的相关信息
            self.start_epoch = 0
            self.initialize()
